  ...
  <>
    ...
    <main className="bg-background text-foreground min-h-screen selection:bg-tiffany selection:text-black">
      ...
      <section className="relative h-screen flex flex-col justify-end pb-32 px-8 md:px-16 overflow-hidden">
        ...
        <motion.div {...fade} className="relative z-10"><p className="text-sm tracking-[0.3em] text-white/50 uppercase mb-4">Company</p><h1 className="text-4xl md:text-7xl font-bold tracking-tighter text-heading-xl">eDrive Careers</h1><p className="text-xl text-white/60 mt-4">Great water vehicles are built by people who believe in what they create.</p></motion.div>
      </section>

      {/* INTRO */}
      <section className="py-32 px-8 bg-black"><div className="max-w-4xl mx-auto"><motion.div {...fade} className="space-y-6 text-xl md:text-2xl font-light leading-relaxed"><p>At eDrive, we design and manufacture next-generation water vehicles.</p><p className="text-muted-foreground">From high-performance JetCars to new categories in water mobility, everything we build starts with people. Our team brings together engineering, craftsmanship, design, production, operations, and vision — all working toward one goal: shaping the future of water mobility.</p></motion.div></div></section>
      ...
      <section id="open-positions" className="py-32 px-8 bg-tiffany text-black">
        <div className="max-w-6xl mx-auto">
          <motion.div {...fade}>
            <h2 className="text-3xl md:text-5xl font-bold tracking-tighter mb-16">Open Positions</h2>
            <div className="bg-black text-white p-8 md:p-12 space-y-10">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-sm">
                {[["Company", job.company], ["Level", job.level], ["Working Model", job.model], ["Remote", job.remote]].map(([l, v]) => (
                  <div key={l}><span className="text-white/40 uppercase text-[10px] tracking-wider">{l}</span><p className="mt-1 text-white/80">{v}</p></div>
                ))}
              </div>
              <div><h4 className="text-sm tracking-[0.2em] text-white/50 uppercase mb-4">About the Role</h4><p className="text-white/80 font-light leading-relaxed">{job.about}</p></div>
              <div className="grid md:grid-cols-3 gap-8">
                <div><h4 className="text-sm tracking-[0.2em] text-white/50 uppercase mb-4">Responsibilities</h4><ul className="space-y-2">{job.responsibilities.map((r, i) => <li key={i} className="flex items-start gap-3 text-white/70 text-sm"><span className="w-1 h-1 mt-2 bg-tiffany rounded-full flex-shrink-0" />{r}</li>)}</ul></div>
                <div><h4 className="text-sm tracking-[0.2em] text-white/50 uppercase mb-4">Requirements</h4><ul className="space-y-2">{job.requirements.map((r, i) => <li key={i} className="flex items-start gap-3 text-white/70 text-sm"><span className="w-1 h-1 mt-2 bg-tiffany rounded-full flex-shrink-0" />{r}</li>)}</ul></div>
                <div><h4 className="text-sm tracking-[0.2em] text-white/50 uppercase mb-4">What We Offer</h4><ul className="space-y-2">{job.offers.map((o, i) => <li key={i} className="flex items-start gap-3 text-white/70 text-sm"><span className="w-1 h-1 mt-2 bg-tiffany rounded-full flex-shrink-0" />{o}</li>)}</ul></div>
              </div>
              <div className="pt-6 border-t border-white/10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                ...
                <a href="mailto:FMC.emotiondrive@gmail.com?subject=Application: Sales Manager (EDR-SALES-DXB-001)" className="px-8 py-3 bg-tiffany text-black text-sm font-bold tracking-widest uppercase hover:bg-white transition-colors">Apply Now</a>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      {/* EXPLORE MORE */}
      <section className="py-24 px-8 bg-black"><div className="max-w-4xl mx-auto text-center"><motion.div {...fade}><h3 className="text-sm tracking-[0.2em] text-white/50 uppercase mb-8">Explore More About eDrive</h3><Link to="/company/profile" className="text-tiffany text-lg hover:text-white transition-colors underline underline-offset-4">Company Profile →</Link></motion.div></div></section>

      {/* CLOSING */}
      <section className="py-40 px-8 text-center bg-black border-t border-white/5"><div className="max-w-4xl mx-auto"><motion.div {...fade}><p className="text-2xl md:text-4xl font-light italic leading-snug">At eDrive, careers are not just positions.<br/>They are part of building something new.</p></motion.div></div></section>
    </main>
    <FooterSection />
  </>
);
